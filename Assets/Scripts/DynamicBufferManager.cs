...
public static class DynamicBufferManager {
  ...
  //Every material has a different bufferEntity list
  private static List<Entity> bufferEntities = new List<Entity>();
  //Contains the index of a bufferEntity inside the bufferEntities list from a material
  private static Dictionary<Material, int> materialEntityBufferID = new Dictionary<Material, int>();
  // Every material has a list of IDs it can distribute to entities
  ...
  /// <summary> 
  /// Add a material to the entity's buffers dynamically. 
  /// New entities require new buffers, this method adds new dynamic buffers for the entity 
  /// </summary>
  ...
  /// <summary> Returns first available ID for a given material. </summary>
  public static int NextIDForEntity(Material material) {
    var ids = availableEntityID[material];
    // Get list of unoccupied indexes from the id list
  ...
  /// <summary> Get the material associated with the entity ID. </summary>
  ...
  private static void CleanBuffer(int bufferID, Entity bufferEntity) {
    ...
    EntityManager.GetBuffer<SpriteLayerBuffer>(bufferEntity).Insert(bufferID, new SpriteLayerBuffer { index = -1 });
